...
public class DatabaseManager {
     ...
     * Cart Table which defines all the columns of this table
     */
    interface FAV_PRODUCT {
        public static String TABLE_NAME = "fav_product";
        public static String _ID = "_id";
        public static String CATEGORY_ID = "category_id";
        public static String PRODUCT_ID = "product_id";
        public static String TOTAL_PRICE = "total_price";
    ...
    public boolean saveCartProduct(Product myCartProduct) {
        ...
        try {
            ...
            System.out.println("DatabaseManager.saveCartProduct===" + contentValues);
            if (productId == -1) {
                return false;
            }
            return true;
    ...
    public boolean saveFavProduct(Product myCartProduct) {
        SQLiteDatabase database = null;
        try {
            DatabaseHelper dbHelper = new DatabaseHelper(ApplicationController.getInstance());
            database = dbHelper.openDataBase();
            ContentValues contentValues = new ContentValues();
            contentValues.put(FAV_PRODUCT.CATEGORY_ID, myCartProduct.categoryId);
            contentValues.put(FAV_PRODUCT.PRODUCT_ID, myCartProduct.productId);
            contentValues.put(FAV_PRODUCT.TOTAL_PRICE, myCartProduct.totalPrice);
            long productId = database.insert(FAV_PRODUCT.TABLE_NAME, null, contentValues);
            System.out.println("DatabaseManager.saveFavProduct===" + productId);
            if (productId == -1) {
                return false;
            }
            return true;
    ...
    public int getFavCount() {
        ...
        try {
            DatabaseHelper dbHelper = new DatabaseHelper(ApplicationController.getInstance());
            database = dbHelper.getReadableDatabase();
            Cursor cursor = database.query(FAV_PRODUCT.TABLE_NAME, null, null, null, null, null, null);
            return cursor.getCount();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public ArrayList<Product> getFavProducts() {
        ...
        try {
            ...
            final String MY_QUERY = "SELECT * FROM fav_product cp, product p WHERE  cp.product_id = p.product_id";
            cursor = database.rawQuery(MY_QUERY, null);
    ...
    public boolean deleteFavProduct(Product myCartProduct) {

        SQLiteDatabase database = null;
        try {
            DatabaseHelper dbHelper = new DatabaseHelper(ApplicationController.getInstance());
            database = dbHelper.openDataBase();
            String whereClause = FAV_PRODUCT.PRODUCT_ID + " = ?";
            String[] whereArgs = {"" + myCartProduct.productId};
            int deleted = database.delete(FAV_PRODUCT.TABLE_NAME, whereClause, whereArgs);
            if (deleted == 0) {
                return false;
            }
            return true;
