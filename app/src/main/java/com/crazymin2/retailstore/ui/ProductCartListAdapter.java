...
public class ProductCartListAdapter extends ArrayAdapter<Product> {
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        System.out.println("ProductCartListAdapter.getView " + item.isInCart + " in Fav " + item.isInFav + " Name " + item.name);
        if (item.isInCart == true) {
            holder.addCart.setChecked(true);
        } else if (item.isInFav == true) {
            holder.addCart.setChecked(true);
        } else {
            holder.addCart.setChecked(false);
        }
